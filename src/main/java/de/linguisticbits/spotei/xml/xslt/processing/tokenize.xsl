            ...
            <xsl:when test="$CONVENTION='HIAT'">
                ...
                <xsl:analyze-string select="." regex="\(\([^\)]+\)\)">
                    <xsl:matching-substring>
                        <xsl:choose>
                            <!-- a measured pause like so : ((0,9s)) (this is HIAT style) -->
                            <xsl:when test="matches(normalize-space(.), '\(\(\d+,\d+s\)\)')">
                                <tei:pause>
                                    <xsl:variable name="DUMMY_W"><w/></xsl:variable>
                                    <xsl:attribute name="xml:id" select="generate-id($DUMMY_W)"/>
                                    <xsl:attribute name="dur">PT<xsl:value-of select="replace(substring-before(substring-after(., '(('), 's))'), ',', '.')"/>S</xsl:attribute>
         ...
         -->
            ...
            <xsl:otherwise>
                ...
                <xsl:analyze-string select="." regex="\[[^\]]+\]">
                    <xsl:matching-substring>
                        <xsl:choose>
                            <!-- a measured pause like so : [0.9] (this is GENERIC style) -->
                            <xsl:when test="matches(normalize-space(.), '\[\d+\.\d+\]')">
                                <tei:pause>
                                    <xsl:variable name="DUMMY_W"><w/></xsl:variable>
                                    <xsl:attribute name="xml:id" select="generate-id($DUMMY_W)"/>
