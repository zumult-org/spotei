...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- 
        ...
        OUTPUT: an FOLKER transcription (flk or fln) - this is an XML file as written by the FOLKER editor and the OrthoNormal tool 
        =================================================================
        HISTORY:
    -->        
    ...
    <xsl:template match="/">
        <folker-transcription>
            <xsl:if test="/tei:TEI/tei:idno[@type = 'AGD-ID']">
                ...
                <xsl:attribute name="corpus-dgd-id"
                    select="substring(/tei:TEI/tei:idno[@type = 'AGD-ID'], 1, 4)"/>
            </xsl:if>
            <head/>
            <xsl:call-template name="MAKE_SPEAKERTABLE"/>
            <recording>
                <xsl:attribute name="path">
                    <xsl:value-of select="//tei:media[ends-with(upper-case(@url), 'WAV')]/@url"/>
                </xsl:attribute>
            </recording>
    ...
    <xsl:template match="tei:seg">
        ...
        <xsl:if test="$TRANSCRIPTION_SYSTEM = 'HIAT'">
            <xsl:choose>
                <xsl:when test="@subtype = 'declarative'">
                    <p>.</p>
                </xsl:when>
                <xsl:when test="@subtype = 'interrogative'">
                    <p>?</p>
                </xsl:when>
                <xsl:when test="@subtype = 'exclamative'">
                    <p>!</p>
                </xsl:when>
                <xsl:when test="@subtype = 'interrupted'">
                    <p>&#x2026;</p>
                </xsl:when>
                <xsl:when test="@subtype = 'not_classified'">
                    <p>&#x02D9;</p>
                </xsl:when>
            </xsl:choose>
        </xsl:if>
    ...
    <xsl:template match="tei:anchor">
        ...
        <xsl:if
            test="not(preceding::tei:anchor[@synch = $SYNCH and ancestor::tei:annotationBlock/@xml:id = $PARENT-ID])">
    ...
    <xsl:template match="tei:body/tei:incident">
        <contribution parse-level="2">
            <xsl:attribute name="id" select="concat('c_NP_', @xml:id)"/>
            <xsl:attribute name="start-reference" select="@start"/>
            <xsl:attribute name="end-reference" select="@end"/>
            <time>
                <xsl:attribute name="timepoint-reference" select="@start"/>
            </time>
            <non-phonological>
                <xsl:attribute name="id" select="@xml:id"/>
                <xsl:attribute name="description" select="tei:desc"/>
            ...
            <time>
                <xsl:attribute name="timepoint-reference" select="@end"/>
            </time>
        </contribution>
    </xsl:template>
    ...
    <xsl:template match="tei:body/tei:pause">
        <!--  
            ...
            -->
        <contribution parse-level="2">
            <xsl:attribute name="id" select="concat('c_P_', @xml:id)"/>
            <xsl:attribute name="start-reference" select="@start"/>
            <xsl:attribute name="end-reference" select="@end"/>
            <time>
                <xsl:attribute name="timepoint-reference" select="@start"/>
